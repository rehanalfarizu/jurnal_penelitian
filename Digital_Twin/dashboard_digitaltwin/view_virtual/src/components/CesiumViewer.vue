...
async function initCesium() {
  ...
  try {
    ...
    if (token) {
      ...
      // Load Cesium 3D Tiles from Ion asset (global city / terrain model)
      try {
        const tileset = await Cesium.Cesium3DTileset.fromIonAssetId(4786731)
        viewer.scene.primitives.add(tileset)
        viewer.zoomTo(tileset)
        console.log('✅ Cesium 3D Tiles loaded from Ion Asset #4786731')
      } catch (tileErr) {
        console.warn('⚠️ 3D Tiles failed to load:', tileErr.message)
      }
    }
